...
const incomeInput: HTMLInputElement | null = document.getElementById('income') as HTMLInputElement | null;
const hourlyIncomeInput: HTMLInputElement | null = document.getElementById('hourly_income') as HTMLInputElement | null;

// Ожидать изменения значения в поле дохода в месяц
if (incomeInput && hourlyIncomeInput) {
    incomeInput.addEventListener('input', () => {
        const monthlyIncome: number = parseFloat(incomeInput.value);
        const workingHoursPerMonth: number = 160; // Предполагаем 160 рабочих часов в месяц
        const hourlyIncome: number = monthlyIncome / workingHoursPerMonth;

        // Округлить значение до двух знаков после запятой
        hourlyIncomeInput.value = hourlyIncome.toFixed(2);
    });
}
